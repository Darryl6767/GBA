/*
 * koopa.c
 * program which demonstrates a Koopa going through a maze
 * walls are gray and path is green
 * the maze border is blue
 */
...
/* ASSEMBLY FUNCTIONS*/
int collision_count(int collisions);
int item_count(int items);
...
int koopa_up(struct Koopa* koopa) {
    /* face up */
    sprite_set_vertical_flip(koopa->sprite, 0);
    koopa->move = 1;

    /* if we are at the top end, just scroll the screen */
    if (koopa->y < koopa->border) {
        return 1;
    } else {
        /* else move up */
        koopa->y++;
...
int is_collision(int x, int y, int xscroll, int yscroll,
        const unsigned short* tilemap, int tilemap_w, int tilemap_h) {
    unsigned short tile;
	tile = tile_lookup(x,y,xscroll,yscroll,tilemap,tilemap_w,tilemap_h);
	if (tile == 0x0079){
		return (1);
	} else{
		return (0);
...
int is_item(int x, int y, int xscroll, int yscroll,
        const unsigned short* tilemap, int tilemap_w, int tilemap_h) {
    unsigned short tile;
	tile = tile_lookup(x,y,xscroll,yscroll,tilemap,tilemap_w,tilemap_h);
	if (tile == 0x007b){
...
int main() {
    ...
    *display_control = MODE0 | BG0_ENABLE | SPRITE_ENABLE | SPRITE_MAP_1D;
	...
	while (1) {
        /* update the koopa */
        koopa_update(&koopa);
		if (is_collision(koopa.x + 1, koopa.y, xscroll, yscroll, maze, maze_width, maze_height) == 1){
			if (collision_count(collisions)%10 == 0)
			{
				char msg [32] = "You have Made 10 Collisions!";
				set_text(msg, 0, 0);
			}
		...
		if (is_item(koopa.x + 1, koopa.y, xscroll, yscroll, maze, maze_width, maze_height) == 1){
			if (item_count(items) >= 1)
			{
				char msg [32] = "You have collected an item!";
				set_text(msg, 0, 0);
			}
		}
